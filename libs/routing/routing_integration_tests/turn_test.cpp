...
UNIT_TEST(Russia_HugeRoundabout_TurnTest)
{
  ...
  TEST_EQUAL(result, RouterResultCode::NoError, ());
  integration::TestTurnCount(route, 2 /* expectedTurnCount */);
  integration::GetNthTurn(route, 0).TestValid().TestDirection(CarDirection::EnterRoundAbout).TestRoundAboutExitNum(7);
  integration::GetNthTurn(route, 1).TestValid().TestDirection(CarDirection::LeaveRoundAbout).TestRoundAboutExitNum(7);
}
...
UNIT_TEST(Russia_Moscow_BolshoyKislovskiyPerBolshayaNikitinskayaUl_TurnTest)
{
  ...
  /// For some reason it contains only one possible turn (+90), but it is expected that it will be two of them (-90 and
  /// +90). This is the reason why the RightTurn is discarded.
  TEST_EQUAL(result, RouterResultCode::NoError, ());
  integration::TestTurnCount(route, 1 /* expectedTurnCount */);
  integration::GetNthTurn(route, 0).TestValid().TestDirection(CarDirection::TurnRight);
}
...
UNIT_TEST(Russia_Kubinka_TurnTest)
{
  ...
  integration::TestTurns(*routeResult.first, {CarDirection::TurnLeft, CarDirection::TurnLeft, CarDirection::TurnLeft,
...
UNIT_TEST(Netherlands_Barneveld_TurnTest)
{
  ...
  // Even if there is no other options, I think that we should show left-turn here
  // https://www.openstreetmap.org/node/286040021
...
void TestNoTurns(ContT const & cont)
{
...
UNIT_TEST(UK_Junction_Circular)
{
  ...
  TRouteResult const routeResult =
      CalculateRoute(GetVehicleComponents(VehicleType::Car), mercator::FromLatLon(53.53692, -2.28832), {0., 0.},
...
UNIT_TEST(Integrated_TurnTest_IncludeServiceRoads)
{
  ...
  for (auto const & s : arr)
  {
    using namespace integration;
    TRouteResult const routeResult =
        CalculateRoute(GetVehicleComponents(VehicleType::Car), mercator::FromLatLon(s.start), {0., 0.},
